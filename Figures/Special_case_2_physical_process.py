...
the values of a physical process subject to a periodicity constraint and the 
stochastic Laplace equation Delta f = w with w white noise.This produces a figure 
showcasing  the kernel inference procedure and its uses as detailed in the case 
example nr 2 which deals with applications featuring boundary conditions on the
kernels. More details can be found in the paper:
...
# i) Introduce constrained behavior
...
K_constraint=spla.block_diag(np.eye(n-2),np.zeros([1,1]))

n_c=np.shape(K_constraint)[0]


# ii) Solve A_c K_x A_c.T=K_c

K_x=lina.pinv(A_constraints)@K_constraint@lina.pinv(A_constraints).T


# iii) Define prior covariance function
...
[U_p,Lambda_p,V_p]=lina.svd(K_sqexp,hermitian=True)
U_p_cut=U_p[:,:n_exp]
...
A_phi=A_constraints@U_p_cut
A_full=np.kron(A_phi,A_phi)
b=np.reshape(K_constraint,[n_c**2])

gamma_0=lina.pinv(A_phi)@K_constraint@(lina.pinv(A_phi.T))
#gamma_0=sf.Get_S_psi(U_p_cut,K_sqexp,A_full,b)


# iii) Execute inference

import KI
beta, mu, gamma, C_gamma, KI_logfile = KI.Kernel_inference_homogeneous(x_measured,gamma_0,Psi,r, A=A_full, max_iter=100)
...
# Save the figure
# plt.savefig('Special_case_2a_physical_process',dpi=400)
...
# Save the figure
# plt.savefig('Special_case_2b_physical_process',dpi=400)
...
# Save the figure
# plt.savefig('Special_case_2c_physical_process',dpi=400)
